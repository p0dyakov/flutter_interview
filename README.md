...
7.  `setState()` вызывается импеоаьтвно для перерисовки
8.  `deactivate()`
9.  `dispose()` вызывается при удалении из дерева
10.  `mounted is false`
...
---
### Что такое сливеры?
`Sliver` - скроллируемая область. Соединяются с помощью `CustomScrollView` 
