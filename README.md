...
`@immutable` говорит о том, что каждая переменная в классе должна быть final
...
---
### Что такое Widget?  
`Widget` - это неизменяемое описание части пользовательского интерфейса. Виджет связан с элементом, который управляет рендерингом. Изменяемое состояние виджета может быть описано некоторой сущностью, но связано оно не с виджетом, а с элементом, который представляет этот виджет.

---
### Что такое Element?  
`Element` - это представление виджета в определенном месте дерева. Управляют жизненым циклом, связывают виджеты и объекты рендеринга.
...
`RenderObject` - это объект дерева визуализации. У него есть родительский объект, а также поле с данными, которое родительский объект использует для хранения специфичной информации, касающейся самого этого объекта, например, его позицию. Данный объект отвечает за  отрисовку, учёт размеров и ограничений.
...
`BuildContext` - это интерфейс, определяющий ряд геттеров и методов, которые могут быть реализованы элементом. 
...
- Обратиться к дереву. Это используется фактически всеми виджетами, которые обычно реализуют метод of (например, `MediaQuery.of(context)`, `Theme.of(context)` и т.д.)

---
### Какие есть проблемы во Flutter?  
- Проблема с подторамживанием анимаций
- [Lightweight Flutter Engines](https://github.com/flutter/flutter/issues/72009)

---
### Что такое InheritedWidget?  
`InheritedWidget` — это виджет, который предоставляет своим потомкам взаимодействовать с данными, хранящимися в нём

---
### Как достать InheritedWidget?
context.dependOnInheritedWidgetOfExactType<T extends InheritedWidget>()
...
---
### Что такое Future?  
`Future` - это класс, который позволяет писать асинхронный код. Код Future НЕ выполняется параллельно, он выполняется в последовательности, определяемой Event Loop.
...
- Взаимодействие между разными Изолятами реализовано посредством сообщений
...
Архитектура - это набор решений по организации программы. Таких, как деление программы на слои, построение связей между ними, выбор state-мэнэджера и т.д. 
...
`Части:`
- `View` получение, обработка, отображение данных

---
###  State-management архитектуры
**Redux**
...
[Подробнее](https://habr.com/ru/company/piter/blog/503074/)
...
- Реактивность. Нет необходимости в вызове дополнительных методов, как в примере со Scoped Model notifyListeners().

`Минусы`
- Порог вхождения чуть выше  
	
[Подробнее](https://habr.com/ru/post/438574/)  
...
[Подробнее](https://habr.com/ru/post/474968/)  
...
`Минусы ` 
- Зависимость от сторонней библиотеки.
- Если модель станет достаточно сложной, будет тяжело уследить, когда действительно необходимо вызывать метод notifyListeners(), чтобы не допускать лишних пересозданий.  
	
[Подробнее](https://habr.com/ru/post/438574/)

---
### Что такое Dependency Injection? Как его делать?  
`Dependency injection (DI)` - это механизм, который позволяет сделать взаимодействующие в приложении объекты слабосвязанными с помощью интерфейсов. Это делает всю систему более гибкой, более адаптируемой и расширяемой. Можно реализовать с помощью библиотек `get_it` или `flutter_simple_dependency_injection`
...
`Плюсы`
 - Прост в использовании 
 - Подходит для хранения небольшого количества данных  
...
`Канал платформы` — это объект, который объединяет имя канала и кодек для кодирования сообщений в двоичную форму и обратно. Нужны для глубокой интеграцией с нативом (уведомления, датчики, камера, аккумулятор, геолокация, звук, подключение, запуск других приложений, информация об устройстве и др.).    
...
`Каналы событий` — это специализированные каналы платформы, предназначенные для использования в случае представления событий платформы Flutter в виде потока Dart.
